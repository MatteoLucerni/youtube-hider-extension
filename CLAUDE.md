...
- `hideInterfaceElements` (default `false`, popup label "Hide on-page controls"): hides the header settings button (desktop, `www.youtube.com`, only; no positioning setting, since it lives in YouTube's own header layout rather than being a draggable overlay) and both whitelist button variants, without pausing any actual filtering. This is the only visibility control for the header button; there is deliberately no separate per-element enable toggle for it.
- `tutorialCompleted` (default `false`): gates the first-run welcome card + spotlight tour.

`FILTER_REAPPLY_KEYS` / `WHITELIST_REAPPLY_KEYS` in `content/state.js` decide which pref changes trigger a full re-filter pass (`resetAppliedFilters` + `startHiding`) via `chrome.storage.onChanged`. When adding a new setting that affects filtering, add its key there too, or a live change in the popup/header dropdown won't take effect until the next page load.
