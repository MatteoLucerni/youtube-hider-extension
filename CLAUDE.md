...
- `headerButtonEnabled` (default `true`): shows the header settings button (desktop, `www.youtube.com`, only). No positioning setting, since the button lives in YouTube's own header layout rather than being a draggable overlay.
- `hideInterfaceElements` (default `false`, popup label "Hide on-page controls"): hides the header button and both whitelist button variants, without pausing any actual filtering.
