...
1. `content/page-bridge.js` — **MAIN world**, `document_start`. Runs in the page's own JS context (not the isolated content-script world) so it can read `window.ytInitialData` and intercept `fetch`/`XMLHttpRequest` calls to YouTube's internal `/youtubei/v1/` endpoints. It walks that JSON to build a video-id → channel-handle cache (covering renderer shapes the visible DOM doesn't always expose a channel link for) and publishes it as a `data-yt-hider-channel-cache` attribute on `<html>`, since a MAIN-world script cannot call into the isolated-world content scripts directly. `content/parsers.js`'s `readChannelCacheFromDOM` reads that attribute back on the isolated side.
2. The isolated-world bundle, `document_idle`, loaded in this exact order — **load-bearing**, since these are classic scripts sharing one global scope and later files reference symbols declared in earlier ones:
...
- `hideInterfaceElements` (default `false`, popup label "Hide on-page controls"): hides the floating button and both whitelist button variants and the logo on dimmed overlays, without pausing any actual filtering.
