...
- The Minimum Views Filter slider offers 38 thresholds instead of 18, so a threshold can be placed where it is actually wanted rather than at the nearest of a handful of stops. The old ladder jumped straight from 100 to 500 views and from 1M to 10M; the new one steps 1, 1.5, 2, 2.5, 3, 5 and 7.5 through every decade from 50 views up to 10M. Every threshold the old slider could produce is still on the new one, so an existing setting keeps its exact value
- The Upload Date Filter's two sliders gain the same treatment. **Hide newer than** goes from 16 stops to 29 and **Hide older than** from 13 to 24, filling in the gaps the old ladders left: 2 and 3 hours, 2 and 5 and 10 days, 3 weeks, 45 days, 4 and 9 months, 18 months, and 3, 4 and 7 years. As with the views filter, every threshold the old sliders could produce is still there, so existing settings keep their exact value
- Both Upload Date sliders are full width as well, and the whole card is laid out in one column. The two sliders used to be pinned to 200px in the left half of the card with the per-page toggles squeezed into a narrow column beside them, which is also why those toggles' tooltips had to open leftwards. The toggles now sit under the sliders in a full-width list, the same shape as every other filter card, and their tooltips open normally
- All four slider boxes, Watch Percentage, Minimum Views and the two Upload Date ones, now use the full width of their card instead of stopping at 260px. The extra travel is what makes the finer thresholds practical to hit. The Watch Percentage slider keeps its 5% steps, it just has more room to place them
- youtubehider.com is restyled to match the extension. The site was a light theme with a blue banner and a dark footer, which looked like a different product from the menu it was advertising; it now uses the same background, text, border and accent colors as the settings menu, and the same single corner radius on every card, button and icon badge, where each page had previously invented its own scale ranging from 4px to 50px. The homepage title picks up the menu's two-tone gradient, feature icons sit in the same tinted rounded badge the setting cards use, and sections are separated by hairline rules over one background instead of alternating grey bands. The 404 page, the first-install welcome page and the feedback widget got the same treatment. The install button in the header no longer pulses forever, and every page now honours the reduced-motion setting
- The site's sections each have their own depth instead of sharing one flat background. The page steps through three background tones rather than repeating one, each section carries a large soft glow in blue or green placed differently from its neighbours, cards are lit from the top so they lift off the band behind them, and the hero carries a faint dot grid that fades out before the edges. Section boundaries are hairlines that fade at both ends rather than full-width rules
- The Support button on the site now opens the same panel as the one in the extension's settings menu, explaining why support matters and offering Ko-fi, GitHub and a Chrome Web Store review, instead of jumping straight to Ko-fi. The same panel is reachable from a new Support button in the footer of every page, which replaces the third-party Ko-fi widget that used to sit there: it was a light-themed button that could not be restyled to match anything, and it pulled a script from Ko-fi's servers on every page load
...
- Upload Date Filter stopped applying to new items on the Subscriptions page after the initial batch loaded, because YouTube Polymer updates text node data rather than replacing DOM nodes on infinite scroll. Fixed by adding `characterData: true` to the MutationObserver options
- Lives filter failed to detect live streams in the `yt-lockup-view-model` DOM format introduced in recent YouTube updates. Fixed by adding `ytBadgeShapeThumbnailLive` and `ytBadgeShapeLive` selectors alongside the existing BEM-style ones
